package com.jyzx.helper.ui.activity.face;
 ...
 */
public class FaceEnterActivity extends BaseActivity implements View.OnClickListener {
    private static final String TAG = FaceEnterActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //StatusUtils.Companion.updateHomeStatus(this);
        setContentView(R.layout.activity_face_enter);
        mContext = this;
        addActionLive();
        initView();
        initLicense();
    ...
    private void addActionLive() {
        // 根据需求添加活体动作
        HelperApplication.Companion.getLivenessList().clear();
        HelperApplication.Companion.getLivenessList().add(LivenessTypeEnum.Eye);
        HelperApplication.Companion.getLivenessList().add(LivenessTypeEnum.Mouth);
        HelperApplication.Companion.getLivenessList().add(LivenessTypeEnum.HeadRight);
    }
    ...
    private void initLicense() {
        ...
        // 申请License取得的APPID
        // assets目录下License文件名
        FaceSDKManager.getInstance().initialize(mContext, "helper-face-android-face-android",
    ...
    private boolean setFaceConfig() {
        ...
        int qualityLevel = (int) util.getSharedPreference(Constants.KEY_QUALITY_LEVEL_SAVE, -1);
        if (qualityLevel == -1) {
            qualityLevel = HelperApplication.Companion.getQualityLevel();
        }
        ...
        config.setLivenessTypeList(HelperApplication.Companion.getLivenessList());
        // 设置动作活体是否随机
        config.setLivenessRandom(HelperApplication.Companion.isLivenessRandom());
        // 设置开启提示音
        config.setSound(!HelperApplication.Companion.isOpenSound());
        // 原图缩放系数
    ...
    public void startCollect() {
        if (HelperApplication.Companion.isActionLive()) {
